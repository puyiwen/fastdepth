...
def parse_command():
    ...
    loss_names = ['l1', 'l2']
    data_names = ['nyudepthv2', 'kitti','nyu_reduced']
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='mobilenetv2_concat', choices=model_names,
                        help='model architecture: ' + ' | '.join(model_names) + ' (default: mobilenet)')
    parser.add_argument('--data', metavar='DATA', default='nyu_reduced',
                        choices=data_names,
                        help='dataset: ' + ' | '.join(data_names) + ' (default: nyudepthv2)')
    parser.add_argument('--data_path',
                        type=str,
                        help='path to train data',
                        default='/home/puyiwen/GuidedDecoding/nyu_data.zip')
    parser.add_argument('--test_path',
                        type=str,
                        help='path to test data',
                        default='/home/puyiwen/GuidedDecoding/NYU_Testset')
...
def merge_into_row(input, depth_target, depth_pred):
    print(input.shape)
    rgb = 255 * np.transpose(np.squeeze(input.cpu().numpy()), (0,2,3,1)) # H, W, C
