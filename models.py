...
class MobileNetSkipAdd(nn.Module):
    def __init__(self, output_size, pretrained=True):
        ...
        mobilenet = imagenet.mobilenet.MobileNet()
        if pretrained:
            pretrained_path = '/home/puyiwen/fastdepth/model_best.pth.tar'
            checkpoint = torch.load(pretrained_path)
            state_dict = checkpoint['state_dict']
...
class MobileNetV2SkipAdd(nn.Module):
    def __init__(self, output_size, pretrained=True):

        super(MobileNetV2SkipAdd, self).__init__()
        self.output_size = output_size
        mobilenet =MobileNetV2()
        if pretrained:
            checkpoint = torch.load('/home/puyiwen/fastdepth/pretrained/mobilenetv2_1.0-0c6065bc.pth')
            mobilenet.load_state_dict(checkpoint,strict = False)
        else:
            mobilenet.apply(weights_init)
        # print(mobilenet.features)
        # print(len(mobilenet.features))#18
        for i in range(18):
            setattr( self, 'block{}'.format(i), mobilenet.features[i])
        
        setattr(self, 'conv',mobilenet.conv)
    ...
    def forward(self, x): #(8,3,224,224)
        ...
        # dec 2: enc4 or enc5
        for i in range(18):
            layer = getattr(self, 'block{}'.format(i))
            x = layer(x)
            # print("{}: {}".format(i, x.size()))
            if i==1:
                x1 = x #(8,16,112,112)
            ...
            # elif i==7:
            #     x4 = x #(8,64,14,14)
        layer_conv = getattr(self,'conv')
        x = layer_conv(x)#(8,1280,7,7)
        for i in range(1,6):
            layer = getattr(self, 'decode_conv{}'.format(i))
            x = layer(x) #i=1 (8,64,7,7)
            x = F.interpolate(x, scale_factor=2, mode='nearest')#(8,64,14,14)
            if i==4:
                x = x + x1
            elif i==3:
                x = x + x2
...
class MobileNetV2SkipConcat(nn.Module):
    def __init__(self, output_size, pretrained=True):

        super(MobileNetV2SkipConcat, self).__init__()
        self.output_size = output_size
        mobilenet =MobileNetV2()
        if pretrained:
            checkpoint = torch.load('/home/puyiwen/fastdepth/pretrained/mobilenetv2_1.0-0c6065bc.pth')
...
#         mobilenet = imagenet.mobilenet.MobileNet()
#         if pretrained:
#             pretrained_path = '/home/puyiwen/fast-depth/model_best.pth.tar'
#             #pretrained_path = os.path.join('imagenet', 'results', 'imagenet.arch=mobilenet.lr=0.1.bs=256', 'model_best.pth.tar')
#             checkpoint = torch.load(pretrained_path)
#             state_dict = checkpoint['state_dict']
